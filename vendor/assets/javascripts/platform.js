...
function nearBestPosition(whereId, theObjId, centerOnEl) {
  ...
  if (el) {
    ...
    var left = trueX;
    var top = trueY;
    var barHeight = 35; //($.browser.msie) ? 45 : 35;
    var barWidth = 0; //($.browser.msie) ? 20 : 0;
...
function openBlackPopup(url,width,height,onCloseCallBack,iframeId){
  ...
  bdiv.append("<img src=\"/assets/gantt/closeBig.png\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
...
function createBlackPage(width,height,onCloseCallBack){
  ...
  var bg=$("<div>").attr("id","__blackpopup__");
  bg.css({position:'fixed',top:"0px",paddingTop:"50px", left:0,width:'100%',height:'100%',  backgroundImage:"url('/assets/gantt/black_70.png')"});
  ...
  var img = $("<img src='/assets/gantt/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
  bdiv.append(img);
  img.click( function(){
    bg.trigger("close");
  });
